...
client.on("guildMemberAdd", member => {
  member.roles.add('814019508297400361'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
...
client.on("guildMemberAdd", member => {
    require("moment-duration-format")
      ...
      let register = '814019473186881536'
  ...
  Kayıt edilmek için teyit odasında <@&814019486004281345> yetkililerine teyit vermen yeterli !
...
client.on("guildMemberAdd", member => {
    ...
    const kytsz = member.guild.roles.cache.find(r => r.id === "814019508297400361") 
     var rol = member.guild.roles.cache.get("814019499745345567") // ŞÜPHELİ HESAP ROLÜNÜN İDSİNİ GİRİN
...
client.on("userUpdate", async (stg, yeni) => {
  var sunucu = client.guilds.cache.get('775397892583849994'); // Buraya Sunucu ID
  var uye = sunucu.members.cache.get(yeni.id);
  var ekipTag = "✫"; // Buraya Ekip Tag
  var ekipRolü = "814019498419290112"; // Buraya Ekip Rolünün ID
  var logKanali = "814019528493105192"; // Loglanacağı Kanalın ID
...
client.on("guildMemberAdd", member => {
  let sunucuid = "775397892583849994"; //Buraya sunucunuzun IDsini yazın
  let tag = "✫"; //Buraya tagınızı yazın
  let rol = "814019498419290112"; //Buraya tag alındığı zaman verilecek rolün IDsini yazın
...
member.roles.add(rol)
  const tagalma = new Discord.MessageEmbed()
      ...
      .setTimestamp()
     client.channels.cache.get('814019528493105192').send(tagalma)
}
})
...
client.on("ready", () => {
  client.channels.cache.get("814019487284068382").join();
})