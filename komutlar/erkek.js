...
const tag = '✫'   
const erkekrol = message.guild.roles.cache.find(r => r.id === '814019502807187486') 
const erkekrol2 = message.guild.roles.cache.find(r => r.id === '814019503213903872')
const erkekrol3 = message.guild.roles.cache.find(r => r.id === '814019503587065856')
...
.setColor('#a22a2a')).then(x => x.delete({timeout: 5000}));
if(!erkekrol3) return message.channel.send(new MessageEmbed()
.setAuthor(message.author.tag, message.author.avatarURL({dynamic:true}))
.setDescription(`3. Erkek rolü ayarlanmamış/yanlış id girilmiş kayıt işlemine devam edilemiyor.`)
.setColor('#a22a2a')).then(x => x.delete({timeout: 5000}));
