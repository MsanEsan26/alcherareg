...
if(!['814019486004281345'].some(role => message.member.roles.cache.get(role)) && !message.member.hasPermission('ADMINISTRATOR')) 
...
const tag = '✫'   
const kadınrol = message.guild.roles.cache.find(r => r.id === '814019501472350230') 
const kadınrol2 = message.guild.roles.cache.find(r => r.id === '814019502101495849')
const kadınrol3 = message.guild.roles.cache.find(r => r.id === '814019502219067433')
const kayıtsız = message.guild.roles.cache.find(r => r.id === '814019508297400361')
const genelchat = message.guild.channels.cache.find(c => c.id === '814019492287873054')
const savelog = message.guild.channels.cache.find(c => c.id === '814036115388104714')
...
.setColor('#a22a2a')).then(x => x.delete({timeout: 5000}));
if(!kadınrol3) return message.channel.send(new MessageEmbed()
.setAuthor(message.author.tag, message.author.avatarURL({dynamic:true}))
.setDescription(`3. Kadın rolü ayarlanmamış/yanlış id girilmiş kayıt işlemine devam edilemiyor.`)
.setColor('#a22a2a')).then(x => x.delete({timeout: 5000}));
